function results=freenet_2D(directions,distances,points,datum)
...
results.EV=100*diag(Q_vv*P);  %...redundancy number ROUND THESE NUMBERS INTO INTEGER IN THE REPORT
results.NV=abs(v)./(sigma_0*sqrt(diag(Q_vv))); %standardised residuals
results.GF=-v./(diag(Q_vv*P));
results.GRZW=sigma_0*4.13./(sqrt(diag(Q_vv*P).*diag(P)));

%External reliability
results.EGK=(1-diag(Q_vv*P)).*results.GRZW;
results.EP=(1-diag(Q_vv*P)).*results.GF;
